...
<table class='content'>
<tr><td>
<form id="myform" action="${_CONTEXT_PATH}/Params!insert.action">
<fieldset class="queryBox"><legend>输入条件</legend>
<table class='params'>
	<tr>
		<td align="right"><font class='fclass'>*&nbsp;</font>ID</td><td><input type='text' id='paraId' name='params.paraId' validate='{required:true,maxlength:32}' /></td>
		<td align="right"><font class='fclass'>*&nbsp;</font>分级</td><td><n:select codetype="para_rank" id="paraRank" name='params.paraRank' emptyOption="true" validate="{required:true}"></n:select></td>
	</tr>
	<tr>
		<td align="right"><font class='fclass'>*&nbsp;</font>值</td><td><input type='text' id='paraValue' name='params.paraValue' validate='{required:true,maxlength:255}' /></td>
		<td align="right"><font class='fclass'>*&nbsp;</font>说明</td><td><input type='text' id='remark' name='params.remark' validate='{required:true,maxlength:512}'/></td>
	</tr>
</table>
</fieldset>
</form>
...
function execute() {
	...
	var url = "${_CONTEXT_PATH}/sys/params!insert.action";
	//Utils.ajaxSubmit(url,mdata);
	Utils.ajaxSubmit(url,mdata,function(result){ 
		if (result.retMsg) {
			$.dialogBox.info('新增成功',function() {
				$.dialogBox.opener.execute();//子页面调用父页面方法
				$.dialog.close();
			    } ); 
		}
	  });
