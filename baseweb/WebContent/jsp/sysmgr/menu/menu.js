...
function addTreeNode() {
	...
	$.dialogBox.alert("请输入菜单ID <input type='text' id='newmenuid'/>",function () {
		var value = $("#newmenuid").val();
		if (!pnode) {
	...
	if (nodes && nodes.length>0) {
		if (nodes[0].childs && nodes[0].childs.length > 0) {
			...
			try{
				...
				var url = "menu!deleteMenuModel.action?"+params;
				$.ajax({
					type: "POST",
					...
					success: function (resData) {
						...
						if(resData.state=='0'){
							$.dialogBox.error('删除失败');
						}
					},
					error: function (resData) {
						$.dialogBox.error('删除失败。请检查网络连接。。。');
					}
				});
...
function displayTreeNode(data) {
	...
	$('#hasChildFlag').val(data.hasChildFlag);
}
function treeNodeLoadData(treeId, treeNode) {
	try{
		...
		var params = {"menuId":menuId,"nodeId":nodeId};
		var url = "menu!queryTreeNodeModel.action";
		Utils.ajaxSubmit(url,params, function (msg) {
			treeNode.data = msg;
			displayTreeNode(msg);
		},function (msg) {
			treeNode.data = msg;
...
function getMenuUrl(treeId,treeNode) {
	try {
		...
		return "menu!getSubMenuModel.action?" + param;
	}
	catch(e) {
		return "menu!getSubMenuModel.action?nodeId=null&menuId=null";
	}
}
function beforeExpand(treeId, treeNode) {
	...
	if (!treeNode.isAjaxing) {				
		...
		$.dialogBox.warn('正在下载数据中，请稍后展开节点。。。');	
		return false;
	}
}
function onAsyncSuccess(event, treeId, treeNode, msg) {
		if (!msg || msg.length == 0 || !zTree) {
			return;
		}
