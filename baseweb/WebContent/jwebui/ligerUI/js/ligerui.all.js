        ...
        return this.each(function() {
            ...
            var g = {
                start: function(e) {
                    ...
                    g.current = {
                        target: g.target,
//                        left: g.target.attr('clientLeft'),
//                        top: g.target.attr('clientTop'),
...
(function($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        pageSizeOptions: [10, 20, 30, 40, 50,100],  //可选择设定的每页结果数
        parms: [],                         //提交到服务器的参数
        buttons:[],                           //按钮
        /**
         *  text     名称
        ...
        onBeforeToggle:null//树节点展开或收缩前触发事件
        }
        json格式：
    ...
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        pageStatMessage: '每页 {pagesize}条,当前 {from} 到 {to},总数 {total} 条 。',
    ...
    $.ligerAddGrid = function(grid, p)
    {
     ...
     if (p.navbtn){
        gridhtmlarr.push("            <div class='l-bar-textc'>每页</div>");
        gridhtmlarr.push("            <div class='l-bar-group l-bar-selectpagesize'></div>");
        gridhtmlarr.push("                <div class='l-bar-textc'>记录</div>");
        ...
        gridhtmlarr.push("                </div>");
        gridhtmlarr.push("                <div class='l-bar-separator'></div>");
        gridhtmlarr.push("                <div class='l-bar-group'><span class='pcontrol'>当前<input type='text' size='4' value='1' style='width:20px' maxlength='3' />页/共 <span></span> 页</span></div>");
        gridhtmlarr.push("                <div class='l-bar-separator'></div>");
        gridhtmlarr.push("                <div class='l-bar-group'>");
        ...
        gridhtmlarr.push("                </div>");
        gridhtmlarr.push("                <div class='l-bar-separator'></div>");
        gridhtmlarr.push("                <div class='l-bar-textc'><span></span></div>");
     }
//        gridhtmlarr.push("                <div class='l-bar-group'>");
//        gridhtmlarr.push("                     <div class='l-bar-button l-bar-btnload'><span></span></div>");
//        gridhtmlarr.push("                </div>");
//        gridhtmlarr.push("                <div class='l-bar-separator'></div>");
        gridhtmlarr.push("                <div class='l-bar-group l-bar-right'><span class='l-bar-text'></span><span class='l-bar-btnline'>");
        $(p.buttons).each(function(i,item) {
            gridhtmlarr.push("<input id='");
            gridhtmlarr.push(item.name);
            gridhtmlarr.push("' type='button' value='");
            gridhtmlarr.push(item.text);
            gridhtmlarr.push("' class='");
            gridhtmlarr.push(item.clazz);
            if (item.enabled == undefined) {
            }
            else if (item.enabled != 'true') {
                gridhtmlarr.push("' disabled='disabled");
            }
            gridhtmlarr.push("'/>");
        });
        gridhtmlarr.push("</span></div>");
        ...
        var g = {
            ...
            enumDisplay: function(item,rowindex,columnName,codetype) {
                var ctObj = p.enumlist[codetype];
                var val = item[columnName];
                if (ctObj) {
                    for (var x = 0 ; x < ctObj.length ; x++) {
                        if (ctObj[x][0] == val) {
                            return ctObj[x][1];
                        }
                    }
        ...
        var po = {
            ...
            initEven: function()
            {
                ...
                $(grid).click(function(e)
                {
                    ...
                    if ($(obj).hasClass("l-grid-tree-link"))
                    {
                        ...
                        if (p.tree.onBeforeToggle) {
                            var rowid = $(rowObj).attr("rowid");
                            var rowdata = g.getRow(rowid);
                            p.tree.onBeforeToggle(rowdata,rowid,rowObj);
                        }
                        g.toggle(rowObj);
                        return;
                    }
            ...
            getCellContent: function(rowData, rowindex, value, column, tree, level)
            {
                ...
                else if (column.codetype){
                    content = g.enumDisplay(rowData,rowindex,column.name,column.codetype);
                }
                else if (column.type == 'date')
                {
            ...
            buildPager: function()
            {
                ...
                $('.l-bar-textc span', g.toolbar).html("共&nbsp;"+ p.total + "&nbsp;条");
    ...
    $.ligerMenu = function (p)
    {
        ...
        var g = {
            ...
            createMenu: function (parentMenuItemID)
            {
                ...
                menu.appendTo('body');
                if ( $.browser.msie && $.browser.version == 6.0) {
                    menu.shadow = $('<div class="l-menu-shadow no-shadow"><iframe style="filter:mask();overflow:hidden;z-index:-1;width:100%;height:100%; " frameborder="0"></iframe></div>').insertAfter(menu);
                    g.updateShadow(menu);
                }
                else
                if (p.shadow)
                ...
                menu.hover(null,function(){
                    if(!menu.showedSubMenu){
                        $("> .l-menu-over:first", menu).css({ top: p.top, left: p.left, width: p.width }); 
                    	$(".l-menu,.l-menu-shadow").hide();
                        $(".l-panel-btn-selected").removeClass("l-panel-btn-selected");
                        $(".l-panel-btn-selected").click();
                    }
                });
                return menu;
            }
