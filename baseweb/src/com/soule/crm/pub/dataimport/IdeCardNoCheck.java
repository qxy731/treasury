...
*/
@Service("check_cardno")
public class IdeCardNoCheck implements IDataImportHandler {
	...
	public void handle(List<ErrorInfoPo> errors, List<HashMap> modellist, List<HashMap> datas) {
		...
		for (int m = 0; m < modellist.size(); m++) {
			...
			if (hm.get("TITLE_NAME").equals("IDE_CARD_NO")) {
		...
		if (pks.size() > 0) {
			for (int i = 0; i < datas.size(); i++) {
				...
				for (int j = 0; j < pks.size(); j++) {
					...
					try {
                        List list = sDefault.getIbatisMediator().find("dataimport.checkCardNo",str);
                        if(list.size() == 0){
