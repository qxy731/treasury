...
@Service("check_org")
public class OrgCodeCheck implements IDataImportHandler {
	...
	public void handle(List<ErrorInfoPo> errors, List<HashMap> modellist, List<HashMap> datas) {
		...
		for (int m = 0; m < modellist.size(); m++) {
			...
			if (hm.get("TITLE_NAME").equals("ORG_CODE")) {
		...
		if (pks.size() > 0) {
			for (int i = 0; i < datas.size(); i++) {
				...
				for (int j = 0; j < pks.size(); j++) {
					...
					try {
                        List list = sDefault.getIbatisMediator().find("dataimport.checkOrg",ss);
                        if(list.size() == 0){
                            addMsg(errors,i,"机构号","值[" +ss+"]未找到");
                        }
